...
type TestMode = 'front-portrait' | 'front-landscape' | 'back-portrait' | 'back-landscape';
type Props = NativeStackScreenProps<RootStackParamList, 'Test'>;

export default function TestScreen({ navigation }: Props) {
  const [mode, setMode] = useState<TestMode>('front-portrait');
  ...
  const portraitProps = {
    ...testProps,
    previewScale: 0.22, // Smaller scale to fit full portrait page on screen
  };
  ...
  const renderComponent = () => {
    switch (mode) {
      case 'front-portrait':
        return <SignFrontPortrait {...portraitProps} />;
      case 'front-landscape':
        return <SignFrontLandscape {...landscapeProps} />;
      case 'back-portrait':
        return <TimeCapsulePortrait {...portraitProps} />;
      case 'back-landscape':
  ...
  return (
    <View style={styles.container}>
      {loading ? (
        ...
        <TouchableOpacity 
          style={[styles.button, mode === 'front-portrait' && styles.activeButton]}
          onPress={() => setMode('front-portrait')}
        >
          <Text style={[styles.buttonText, mode === 'front-portrait' && styles.activeButtonText]}>
            Front Portrait
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity 
          style={[styles.button, mode === 'back-portrait' && styles.activeButton]}
          onPress={() => setMode('back-portrait')}
        >
          <Text style={[styles.buttonText, mode === 'back-portrait' && styles.activeButtonText]}>
            Back Portrait
          </Text>
        </TouchableOpacity>
